                ...
                <div class="overflow-x-auto relative shadow-md sm:rounded-lg w-full">
                    ...
                    <form method="POST" action="{{ route('consulta.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                            <h2 class="text-gray-900 text-lg mb-1 font-medium title-font">{{ __('Escribe tu consulta') }}
                            </h2>
                            <div class="relative mb-4">
                                <label for="motivo"
                                    class="leading-7 text-sm text-gray-600">{{ __('Motivo') }}</label>
                                <input type="text" required id="motivo" name="motivo" value="{{ old('motivo') }}"
                            ...
                            <div class="relative mb-4">
                                ...
                                <input type="text" required required id="diagnostico" name="diagnostico" value="{{ old('diagnostico') }}"
                                    class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                @error('diagnostico')
                                    <p id="standard_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400"><span class="font-medium">{{ $message }}</span></p>
                                @enderror
